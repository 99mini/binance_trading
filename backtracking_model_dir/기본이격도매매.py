...
sand_percentage = binance.fetch_ticker("SAND/USDT")['percentage']
mana_percentage = binance.fetch_ticker("MANA/USDT")['percentage']

sub_sand_mana = abs(sand_percentage - mana_percentage)

print("SAND 증감률 : ", sand_percentage)
print("MANA 증감률 : ", mana_percentage)
print("SAND - MANA 증감률 절대값 : ", sub_sand_mana)


percentage_list = []
for day_idx, day in enumerate(df_sand_day.datetime):
    day = day.strftime('%y%m%d')
    yesterday_close = df_sand_day['open'].iloc[day_idx]
    # print(day," : yesterday_close : ", yesterday_close)
    for data_idx, data in enumerate(df_sand.datetime):
        data = data.strftime('%y%m%d')

        if day == data:
            percentage = (df_sand['close'].iloc[data_idx] - yesterday_close) / yesterday_close * 100
            percentage = round(percentage, 2)
            percentage_list.append(percentage)
        else:
            continue
df_sand['percentage'] = percentage_list

percentage_list = []
for day_idx, day in enumerate(df_mana_day.datetime):
    day = day.strftime('%y%m%d')
    yesterday_close = df_mana_day['open'].iloc[day_idx]
    for data_idx, data in enumerate(df_mana.datetime):
        data = data.strftime('%y%m%d')

        if day == data:
            percentage = (df_mana['close'].iloc[data_idx] - yesterday_close) / yesterday_close * 100
            percentage = round(percentage, 2)
            percentage_list.append(percentage)
        else:
            continue

df_mana['percentage'] = percentage_list
...
plt.title("Basic Model SAND/USDT " + str(TIME_FRAME))
...
plt.title("Basic Model MANA/USDT " + str(TIME_FRAME))
